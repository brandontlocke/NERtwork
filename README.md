...
1. Run [batchner.sh](#batchner.sh) over a folder of text files. It will create a spreadsheet with a count of every named entity in every document in the folder.
...
3. Run [batchner-collapse.py](#batchner-collapse.py) to consolidate duplicate entities creating by refinement. Only use this if you completed step 2.
4. Run [batchner-to-network.csv](#batchner-to-network.csv) to perform a bipartite network projection and save the resulting node and edge lists.
...
* Python 3
* A [batchner](https://github.com/brandontlocke/batchner) output

### Normalizing entities

You may find a number of entities that mean the same thing, but are spelled or differently. For example, you may know that "B. Obama" and "Barack Obama" are the same thing, or that "Chicago, Illinois", "Chicago", and "Chicago, IL" all refer to the same place. You'll want to edit those to make them all the same. If you have a really small dataset, you may be able to edit them in spreadsheet software, but I'd strongly recommend using [OpenRefine](https://openrefine.org/) to locate potential duplicates and edit in bulk. You'll likely want to use the [text facet](https://github.com/OpenRefine/OpenRefine/wiki/Cell-Editing#editing-through-text-facets) and [clustering](https://github.com/OpenRefine/OpenRefine/wiki/Clustering) features. 
...
1. Save [batchner-collapse.py]() to your machine
2. In a terminal window, navigate to the directory with your batcher-to-network.py file and type `python batchner-collapse.py path/to/your/file`
...
Takes a [batchner](https://github.com/brandontlocke/batchner) output, creates a [bipartite projected network](https://en.wikipedia.org/wiki/Bipartite_network_projection) of named entity relationships, and saves node and edge lists as CSV. In other words, it counts the co-occurrence of named entities within a set of documents and represents those co-occurrences as connected nodes in a network.
...
* `-proj_name` allows you to give your project a name that will be at the beginning of the filename. (default = batchnertonetwork)
* `-subset` [optional] allows you to limit your network to only specific types of networks. "none" will create a network. including all types of entities, "person", "location", or "organization" will create a network of only that type of entity, and "all" will create 4 networks: all types, person, location, and organization. (default = none)
...
### Instructions
1. Save the [batchner-to-network.py file](https://github.com/brandontlocke/batchner-to-network/blob/master/batchner-to-network.py) to your machine
2. In a terminal window, navigate to the directory with your batchner-to-network.py file and type `python batchner-to-network.py -i=path/to/your/file`, plus any other flags you may want
...
* `python batchner-to-network.py -i=https://gist.githubusercontent.com/brandontlocke/f568ffdeaf7fdb17e27872916645591b/raw/f1a28d05ce2a4cdbbe11c09404cdf83c7a5fb1c1/inaugural-batchner -proj_name=inauguralspeeches`
	* this will take a file from Github, prefix every file with 'inauguralspeeches_', and will include all entities. **this file is available on the web, so feel free to run tests with this command**
